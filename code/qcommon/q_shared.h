...
#if defined(__GNUC__) || defined(__clang__)
#define QALIGN(x) __attribute__((aligned(bitceil(x))))
#else
#define QALIGN(x)
#endif
...
#define  vec(N,T) QALIGN((N == bitceil(N) ? N : 1) * alignof(T)) __typeof__(__typeof__(T)[N         ])
#define avec(N,T) QALIGN((N                      ) * alignof(T)) __typeof__(__typeof__(T)[bitceil(N)])
